...
protected:
    ...
    // RAII guard to reset audio streamer on scope exit (exception-safe)
    struct AudioStreamerGuard {
        AudioStreamerVariant& ref;
        ~AudioStreamerGuard() { ref = std::monostate{}; }
    };
...
public:
    ...
    VLMDecodedResults generate(
        ...
        AudioStreamerGuard guard{m_pending_audio_streamer};
