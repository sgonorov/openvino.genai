...
InputsEmbedderQwen3Omni::InputsEmbedderQwen3Omni(const VLMConfig& vlm_config,
    ...
    // Load the merged vision model (patch_embed + transformer + merger)
...
ov::Tensor InputsEmbedderQwen3Omni::get_inputs_embeds(
    ...
    if (m_audio_embeddings && m_audio_embeddings.get_size() > 0 && m_audio_token_id >= 0) {
        // Use cached input_ids from parent's get_inputs_embeds() call.
        // Re-tokenizing via get_encoded_input_ids() would corrupt m_cache_state.
        OPENVINO_ASSERT(m_last_input_ids && m_last_input_ids.get_size() > 0,
                        "m_last_input_ids must be populated by parent get_inputs_embeds()");
        merge_audio_embeddings(input_embeds,
                               std::vector<int64_t>(m_last_input_ids.data<int64_t>(),
...
std::pair<ov::Tensor, ov::Tensor> InputsEmbedderQwen3Omni::run_video_image_embeddings_merger(
    ...
    const std::vector<size_t>& videos_sequence) {
    OPENVINO_ASSERT(m_ireq_queue_merged_vision,
                    "Merged vision model not loaded. Ensure openvino_vision_embeddings_model.xml is present.");
